...
We assume you already have a working `langchain` installation, including an LLM and embedding model as well as a [supported vector store](/reference/langchain_graph_retriever/Adapter.qmd).
...
The above creates a graph traversing retriever that starts with the nearest animal (`start_k=1`), retrieves 10 documents (`k=10`) and limits the search to docuemnts that are at most 2 steps away from the first animal (`depth=2`).
